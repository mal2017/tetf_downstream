...
coex_w_dp <- read_tsv("results/analysis/coexpression/filtered_models.tsv.gz") %>%
  filter(gene_symbol == "Dp") %>%
  pull(feature.y) %>%
  unique()

coex_w_pan <- read_tsv("results/analysis/coexpression/filtered_models.tsv.gz") %>%
...
# 6.3, 11.1
svg("~/Downloads/CSHL22_figs/tracks.svg",width =9.5, height = 4.6)
