...
dir.create("~/Downloads/labmeeting_figs")
...
png("~/Downloads/labmeeting_figs/heatmap.png",width = 7.5, height = 6.1,units="in", res=150)
...
ggsave("~/Downloads/labmeeting_figs/ncoex_scatter.png",intro_scatter, height = 4.5, width = 8.5,units="in", dpi=150)
...
g_comm_obs_exp <- comm_obs_exp %>%
  ...
  geom_jitter(aes(color=community),size=5,width = 0.05) +
...
ggsave("~/Downloads/labmeeting_figs/te_comm_obs_exp.png",
       g_comm_obs_exp, width = 4, height = 6,units="in", dpi=150)
...
g_te_comms <- read_rds("results/analysis/direct_binding/coregulated_te_communities.igraph_list.rds")$female_model_01 %>%
  ...
  geom_node_label(data=. %>% filter(sig),aes(label=name,fill=community),check_overlap = T,family="bold",size=rel(5)) +
  #scale_fill_paletteer_d("ggsci::default_igv") +
  scale_fill_manual(values=cols) +
  guides(fill="none") +
  theme_graph()

ggsave("~/Downloads/labmeeting_figs/te_comms.png",
       g_te_comms, width = 11, height = 6,units="in", dpi=150)
...
ggsave("~/Downloads/labmeeting_figs/tf_obs_exp.png",
       intra_grp_dist_all$tf_obs_exp +
         ylab("observed mash dist") +
         xlab("expected mash dist") +
         ggrepel::geom_text_repel(data = . %>% filter(in.dist< 0.45 & gene_symbol %in% tx.related$gene_symbol),aes(label=paste0("italic('",gene_symbol,"')")),
...
(plot_gene_grp_gsea(c("AnimalTFDB 3.0","C2H2 ZINC FINGER\nTRANSCRIPTION\nFACTORS")) + facet_wrap(~pathway,ncol=2)) %>%
  ggsave("~/Downloads/labmeeting_figs/animaltdb_gsea.png", ., width = 9.5, height = 3.7,units="in", dpi=150)

# -- top decile n bar
read_rds("results/plots/plot_tx_related_in_lms.rds") %>%
  ggsave("~/Downloads/labmeeting_figs/tx_related_in_lms_bar.png",.,width = 5.6, height = 4.2,units="in", dpi=150)

# ------------------------------------------------------------------------------
# binding
# ------------------------------------------------------------------------------
ggsave("~/Downloads/labmeeting_figs/with_grp_nullranges.png",read_rds("results/plots/plot_within_tf_grp_nullranges.rds"),height = 4.2, width = 5.2,units="in", dpi=150)
...
ggsave("~/Downloads/labmeeting_figs/pirna_ecdfs.png",.,
       height = 3.2, width = 9,units="in", dpi=150)
...
ggsave("~/Downloads/labmeeting_figs/tfrnai_coex_signature_gsea.nes.png", .,
       width = 9, height =5,units="in", dpi=150)
...
(read_rds("results/plots/plot_ourKD_gsea.plot_list.rds")$plot + 
  ...
  ggsave("~/Downloads/labmeeting_figs/our_kd_gsea.png",plot = .,width = 5, height = 4.2,units="in", dpi=150)

our_kd_volcs <- read_rds("results/plots/plot_this_study_kd_deseq2.gg_list.rds")

(our_kd_volcs$NfI + theme(legend.position = c(0,0.9),legend.justification = "left") + xlab("log2(NfI-RNAi / GFP)")) %>%
  ggsave("~/Downloads/labmeeting_figs/our_kd_NfI.png",.,width = 5, height = 4.2,units="in", dpi=150)
